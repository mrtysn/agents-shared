...
#pane{position:absolute;top:12px;right:calc(22rem - 13px);z-index:3;width:26px;height:26px;border-radius:50%;border:1px solid var(--line);background:var(--card);color:var(--ink2);cursor:pointer;padding:0;display:grid;place-items:center}#pane svg{display:block;transition:none}body.nopane #pane svg{transform:rotate(180deg)}
...
function side(){const a=$('#side');a.replaceChildren();const noProj=!S.project;for(const o of $('#scope').options)o.disabled=o.value!=='user'&&noProj;if(noProj)$('#scope').value='user';else if(!scopeTouched&&$('#scope').value==='user')$('#scope').value='local';
 ...
 if(SELKIND===''||SELKIND==='folder'){const f=FOLDERS.find(x=>x.path===(CTX||ROOT.path))||ROOT;const onG=S.groups.filter(g=>g.enabled).length;
  ...
  a.append(el('h2',{},'Skills'));const tb=el('table',{class:'skills'},el('thead',{},el('tr',{},el('th',{scope:'col'},'Skill'),el('th',{scope:'col'},'Loaded'),el('th',{scope:'col'},'Claude sees it'))));const tbody=el('tbody');for(const k of g.skills){tbody.append(el('tr',{},el('td',{class:'name'},link('s:'+g.name+':'+k.name,k.name,'mono'),k.has_override?el('span',{class:'k',title:'Carries a local override'},' ✎'):''),el('td',{},sw(k.enabled,`${k.name} loaded`,v=>act('/api/skill-enable',{dir:k.dir,enabled:v},`${k.name}: ${v?'on':'off'} in every folder`))),el('td',{},sw(!k.slash_only,`${k.name} visible to Claude`,v=>act('/api/skill',{dir:k.dir,slash_only:!v},`${k.name}: ${v?'auto':'slash-only'}`),['auto','slash']))))}tb.append(tbody);a.append(tb);
