...
<!-- LOCAL: this group vendors only the engine-neutral skills (router, disciplines, genres,
workflows), flat under skills/<name>/. Engine skill sets are not installed; the engine is still
detected so the gap can be stated, per §6. -->
| # | Engine | Primary signal | Skill set root |
|:-:|--------|----------------|----------------|
| 1 | Godot | `project.godot` | not installed |
| 2 | Unreal | `*.uproject` | not installed |
| 3 | Unity | `Assets/` **and** `ProjectSettings/ProjectVersion.txt` | not installed |
| 4 | Bevy | `Cargo.toml` with a `bevy` dependency | not installed |
| 5 | Phaser | `package.json` dep `phaser` | not installed |
| 6 | PixiJS | `package.json` dep `pixi.js` | not installed |
| 7 | three.js | `package.json` dep `three` | not installed |
| 8 | LÖVE | `conf.lua` / `main.lua` calling `love.*` | not installed |
| 9 | pygame | `*.py` with `import pygame` | not installed |
| 10 | Roblox | `*.rbxl(x)` / `*.project.json` (Rojo) | not installed |
<!-- LOCAL END -->
...
### 3a. Engine skills — not installed in this group

<!-- LOCAL: the upstream engine skill sets (Godot, Unity, Unreal, web and other engines) are not
vendored here. Where a row below or a genre's `composes` names one, the discipline skill owns the
concept and the engine API comes from the project itself; state the gap as §6 describes. -->
Upstream ships per-engine skills (`godot-*`, `unity-*`, `unreal-*`, `phaser-*`, `pixijs-*`,
`threejs-*`, `bevy-ecs`, `pygame-core`, `love2d-core`, `roblox-*`). None is installed. Treat every
engine binding named below as "engine known but no skill covers the subtask" (§6).
<!-- LOCAL END -->
...
1. **Preloaded:** only this router's `name` + `description` are in context; the other skills in
   this group are hidden from the listing. <!-- LOCAL: descriptions are not preloaded; the
   tables above are the complete index. --> Decide from the tables above plus the fingerprint —
   do **not** pre-read bodies.
2. **On selection:** read the body of **each chosen** `skills/<name>/SKILL.md` — and
...
3. Only if engine choice actually blocks the answer, ask **one** targeted question ("Which engine —
   Godot, Unity, Unreal, or a web/other engine?"). <!-- LOCAL: no engine set is installed, so
   there is no covered default to fall back on. --> Do not default to an engine; state that no
   engine skill is installed and work from the discipline skill.
