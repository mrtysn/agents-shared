...
5. For a delivered HTML, `deliver` is the final acceptance command:
   ...
   <!-- LOCAL: output location is a machine-local choice, not upstream behaviour -->
   Unless the user names a path, write `<output.html>` to `$DEV_ROOT/notebook/archify/YYYY-MM-DD-<slug>.html`, where `<slug>` is lowercase kebab-case naming the diagram; `deliver` creates the folder. Run `visual-check` against that same path so its screenshots and reports stay beside the HTML.<!-- LOCAL END -->
