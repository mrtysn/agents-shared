...
1. **Never modify source code.** <!-- LOCAL: output goes to animation-plans/, so it never collides with shadcn/improve's plans/ -->The only files you create or edit live under `animation-plans/`.<!-- LOCAL END --> If asked to "just fix it", decline and point to `improve-animations execute <plan>` or to running the plan with any agent.
...
One plan per selected finding, using [PLAN-TEMPLATE.md](PLAN-TEMPLATE.md), written into <!-- LOCAL: animation-plans/ -->`animation-plans/`<!-- LOCAL END --> as `NNN-short-slug.md` (monotonic numbering; respect existing plans). Stamp each plan with the current commit (`git rev-parse --short HEAD`).
...
Finish by creating or updating <!-- LOCAL: animation-plans/ -->`animation-plans/README.md`<!-- LOCAL END -->: recommended execution order, dependencies between plans, and a status column.
...
| `reconcile` | Re-check <!-- LOCAL: animation-plans/ -->`animation-plans/`<!-- LOCAL END --> against the current code: mark done plans DONE, refresh stale file:line references, retire fixed findings |
